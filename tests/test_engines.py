...
def test_d16z6_rpm_axis_hits_idle_pocket_and_peaks():
    ...
    assert len(axis) == 16
    assert any(abs(x - 500) < 1 for x in axis)
    assert any(abs(x - 570) < 1 for x in axis)
    assert any(abs(x - 670) < 1 for x in axis)
    assert any(abs(x - 770) < 1 for x in axis)
...
def test_d16z6_fillers_are_multiples_of_50():
    ...
    for x in axis:
        if x in specified or any(abs(x - point) < 1 for point in specified):
            continue
...
def test_overboost_follows_profile_max_not_profile_name():
    s = EngineSpec(boost_psi=10.4)
    assert _overboost_kpa(s) == 200.0
    s2 = EngineSpec(boost_psi=7.0)
