...
def test_application_loads_new_boost_gain_from_engine_profile():
    spec = load_engine_profile("d16z6")
    assert spec.boost_retard_gain == 0.6
    assert spec.boost_timing_limit == 20


def test_application_v2_boost_curve_uses_mirrored_kpa_gain():
...
def test_application_build_table_uses_v2_timing():
