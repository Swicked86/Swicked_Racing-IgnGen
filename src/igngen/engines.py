"""Engine profiles — pre-fill EngineSpec for known motors."""
...
def _engine_dirs(search_dirs: list[Path] | None = None) -> list[Path]:
    ...
    for d in search_dirs or []:
        dirs.append(Path(d))
    # Prefer engines/; keep vehicles/ as a fallback for old checkouts
    dirs.append(Path.cwd() / "engines")
    dirs.append(Path(__file__).resolve().parents[2] / "engines")
    dirs.append(Path.cwd() / "vehicles")
    dirs.append(Path(__file__).resolve().parents[2] / "vehicles")
    return dirs


def list_engines(search_dirs: list[Path] | None = None) -> list[Path]:
    found: dict[str, Path] = {}
    for d in _engine_dirs(search_dirs):
        if not d.is_dir():
            continue
...
def find_engine(name: str, search_dirs: list[Path] | None = None) -> EngineProfile | None:
    key = name.strip().lower().replace(" ", "")
    for d in _engine_dirs(search_dirs):
        for candidate in (d / f"{key}.ini", d / f"{key.upper()}.ini", d / f"{name}.ini"):
            if candidate.is_file():
                return load_engine(candidate)
        if d.is_dir():
...
def load_engine(path: Path) -> EngineProfile:
    cp = configparser.ConfigParser()
    cp.read(path)
    # [profile] preferred; [vehicle] kept as legacy alias in older inis
    meta = {}
    if cp.has_section("profile"):
        meta = cp["profile"]
    elif cp.has_section("vehicle"):
        meta = cp["vehicle"]
    ...
    spec = EngineSpec(
        ...
        idle_pocket_width=f(eng, "idle_pocket_width", 100),
...
def describe_engine(v: EngineProfile) -> str:
    s = v.spec
...
# Back-compat aliases (old import names)
VehicleProfile = EngineProfile
list_vehicles = list_engines
find_vehicle = find_engine
load_vehicle = load_engine
describe_vehicle = describe_engine
